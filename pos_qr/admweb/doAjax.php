...
if (_MODULE_ != '') {
	...
	if ($ty == 'plugin') {
		...
		if (!is_file($mainPageModule)) {
			setRaiseMsg('Cannot get file (/modules/' . _MODULE_ . '/main_' . _MP_ . '.php)', time(), 1);
			$mainPageModule = '';
		}
	} else {
		if (_MP_ != '') {
			$fmodule = PATH_MODULE . '/' . _MODULE_ . '/ajax_' . _MP_ . '.php';
			$fcore = PATH_CORE . '/' . _MODULE_ . '/ajax_' . _MP_ . '.php';
			$mainPageModule = is_file($fmodule) ? $fmodule : $fcore;
		} else {
			$fmodule = PATH_MODULE . '/' . _MODULE_ . '/ajax_' . _MODULE_ . '.php';
			$fcore = PATH_CORE . '/' . _MODULE_ . '/ajax_' . _MODULE_ . '.php';
			$mainPageModule = is_file($fmodule) ? $fmodule : $fcore;
		}

		if (!is_file($mainPageModule)) {
			setRaiseMsg('Cannot get file (/modules/' . _MODULE_ . '/ajax_' . _MP_ . '.php)', time(), 0);
			$mainPageModule = '';
		}
	}
} elseif ($ty == 'ajax' && $inc != '') {
	...
	$mainPageModule = PATH_PLUGIN . '/' . $pg . '/ajax_' . $inc . '.php';
}
