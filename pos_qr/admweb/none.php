...
if (_MODULE_ != '') {
	...
	// include main
	if (_MP_ != '') {
		$fmodule = PATH_MODULE . '/' . _MODULE_ . '/none_' . _MP_ . '.php';
		$fcore = PATH_CORE . '/' . _MODULE_ . '/none_' . _MP_ . '.php';
		$mainPageModule = is_file($fmodule) ? $fmodule : $fcore;
	} else {
		$fmodule = PATH_MODULE . '/' . _MODULE_ . '/none_' . _MODULE_ . '.php';
		$fcore = PATH_CORE . '/' . _MODULE_ . '/none_' . _MODULE_ . '.php';
		$mainPageModule = is_file($fmodule) ? $fmodule : $fcore;
	}

	if (!is_file($mainPageModule)) {
		setRaiseMsg('Cannot get file (/modules/' . _MODULE_ . '/none_' . _MP_ . '.php)', _TIME_, 1);
		$mainPageModule = '';
	}
}
