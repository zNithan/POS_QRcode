...
?>
			<div style="margin-bottom: 10px;">
				<?php echo @$tagname['isContent_icon']; ?>
				<div style="background-color: #EEE;border: 1px solid #CCC;padding: 5px;">
					<?php if ($aArticleRows['content'][$kLang]['content_icon'] == '') {?>
					<input type="file" name="contectIcon[<?php echo $kLang; ?>]">
					<?php } else { ?>
					<a href="<?php echo $ucaf; ?>" style="color: #ff0000;" onclick="return confirm('Delete?');">[ ลบไฟล์นี้ ]</a>
					<a href="<?php echo URL_UPLOAD.'/'.$aArticleRows['content'][$kLang]['content_icon']; ?>" target="_blank"><?php echo $aArticleRows['content'][$kLang]['content_icon']; ?></a>
					<?php } ?>
				</div>
			</div>