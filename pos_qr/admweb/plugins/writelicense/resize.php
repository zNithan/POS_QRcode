<?php
    ...
    */############################################ 
    ...
    { 
        ...
        function resize_thumbnail($imgfile) 
        { 
            //detect image format
            //require_once 'conf.ini.php';
            $extention = explode('.', $imgfile);
            $extention = strtoupper(end($extention));
            $this->img["format"] = $extention;
        ...
        { 
            //height 
            $this->img["tinggi_thumb"]= ($this->img["lebar"] > $size) ? $size : $this->img["lebar"];
        ...
        { 
			...
			$this->img["lebar_thumb"] = ($this->img["lebar"] > $size) ? $size : $this->img["lebar"];
		...
		function uploadImg($file,$path,$filename)
		{
			if ($file['tmp_name'] != '' && $file['error'] == 0) {
				move_uploaded_file($tmp_name, "$upfolder/$name");
			}
		}
    } 
    
    /**
     * $thumb=new resize_thumbnail($strUploadToPath . '/' . $strUploadFileArr['name']);
       $thumb->size_width(111);
       $ok = $thumb->save($strUploadToPath . '/thump_' . $strUploadFileArr['name']);
     */
    ?>